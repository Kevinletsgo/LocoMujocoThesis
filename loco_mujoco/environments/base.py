...
class LocoEnv(MultiMuJoCo):
    ...
    def __init__(self, xml_handles, action_spec, observation_spec, collision_groups=None, gamma=0.99, horizon=1000,
        ...
        self.info.actionParam_space = self.create_actionParam_space()
        ...
        low_param, high_param = self.info.actionParam_space.low.copy(), self.info.actionParam_space.high.copy()
        self.norm_act_mean_param = (high_param + low_param) / 2.0
        self.norm_act_delta_param = (high_param - low_param) / 2.0
        self.info.actionParam_space.low[:] = -1.0
        self.info.actionParam_space.high[:] = 1.0
    ...
    def create_actionParam_space(self):
        dim = 19
        ...
        700, 1, 0,
        1, 1, 1, 1, 1, 1, 1, 1])
        # 创建 Box 空间，shape 维度为 3

        actionParam_space = Box(low=low, high=high, shape=(dim,))
        return actionParam_space
    ...
    def _preprocess_action(self, action):
        ...
        # # 找出绝对值大于1的元素的索引
        # mask = np.abs(action) > 1
        # abs_vals = np.abs(action[mask])
        #
        # # 最小和最大绝对值
        ...
        #
        # # 归一化（带符号）
        # normalized_vals = np.sign(action[mask]) * (abs_vals - min_val) / (max_val - min_val)
        #
        # # 创建新数组，替换归一化后的值
        # normalized_arr = action.copy()
        # normalized_arr[mask] = normalized_vals
